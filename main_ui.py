...
def settings_page(main_content):
    for widget in main_content.winfo_children():
        widget.destroy()

    ctk.CTkLabel(main_content, text="Settings", font=("Inter", 60, "bold"), text_color="#2E86C1").pack(pady=20)

    # Background Color
    ctk.CTkLabel(main_content, text="Background Color", font=("Inter", 20)).pack(pady=(20, 5))
    bg_color_entry = ctk.CTkEntry(main_content, placeholder_text="e.g., white, #f0f0f0")
    bg_color_entry.pack()

    # Font Size
    ctk.CTkLabel(main_content, text="Font Size", font=("Inter", 20)).pack(pady=(20, 5))
    font_size_entry = ctk.CTkEntry(main_content, placeholder_text="e.g., 16")
    font_size_entry.pack()

    # Text Color
    ctk.CTkLabel(main_content, text="Text Color", font=("Inter", 20)).pack(pady=(20, 5))
    text_color_entry = ctk.CTkEntry(main_content, placeholder_text="e.g., black, #333333")
    text_color_entry.pack()

    # Appearance Mode
    ctk.CTkLabel(main_content, text="Appearance Mode", font=("Inter", 20)).pack(pady=(20, 5))
    mode_menu = ctk.CTkOptionMenu(main_content, values=["Light", "Dark", "System"])
    mode_menu.pack()

    def apply_settings():
        bg = bg_color_entry.get().strip()
        fs = font_size_entry.get().strip()
        tc = text_color_entry.get().strip()
        mode = mode_menu.get()

        if bg:
            main_content.configure(fg_color=bg)
        if fs.isdigit():
            for widget in main_content.winfo_children():
                if isinstance(widget, ctk.CTkLabel):
                    widget.configure(font=("Inter", int(fs)))
        if tc:
            for widget in main_content.winfo_children():
                if isinstance(widget, ctk.CTkLabel):
    ...
    ctk.CTkButton(main_content, text="Apply", command=apply_settings).pack(pady=30)
...
def main_window():
    ...
    header = ctk.CTkFrame(master=root, fg_color="white", height=60)
    ...
    sidebar = ctk.CTkFrame(master=root, width=200, fg_color="red")
    sidebar.grid(row=1, column=0, sticky="nsew")

    main_content = ctk.CTkFrame(master=root, fg_color="white")
    ...
    def pomodoro_timer():
        clear_main_content()
        ctk.CTkLabel(main_content, text="Pomodoro Timer Page", font=("Inter", 24, "bold")).pack(pady=20)
